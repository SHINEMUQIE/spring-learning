...
class ConfigurationClassEnhancer {
	...
	private static final Callback[] CALLBACKS = new Callback[] {
			// 增强方法，主要是 bean 得作用域（不用每次都调用new 去实例化一个类）
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		 ...
		 * 增强接口，为什么要增强接口？
		 * 便于判断，表示一个类以及增强了
		 * EnhancedConfiguration 借口继承自 BeanFactoryAware        setBeanFactory(BeanFactory beanFactory)
		 * setBeanFactory(BeanFactory beanFactory) 该方法能够将 bean 添加到 BeanFactory，既 bean 工厂中,
		 * 而 BeanFactory 能够也提供了与之对应的 get 方法，获取一个 bean。所以在这里将 target 目标放到 BeanFactory 中，
		 * 当在次有 target 被生成时，可以直接从 BeanFactory 获取。
		 */
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			// enchancedConfigInstance 代理对象
			// 通过 enchancedConfigInstance 中 cglib 生成的成员变量 $$beanFactory 获得 beanFactory
			...
			// 判断返回的对象是否是一个 FactoryBean
			if (factoryContainsBean(beanFactory, BeanFactory.FACTORY_BEAN_PREFIX + beanName) &&
					factoryContainsBean(beanFactory, beanName)) {
				// 得到 FactoryBean 对象， FACTORY_BEAN_PREFIX == &
				// 从 beanFactory 中拿到一个名称为 "&" + beanName 的一个 FactoryBean 对象
				// 这个 FactoryBean 对象是代理时 getObject() 产生的对象
				...
				if (factoryBean instanceof ScopedProxyFactoryBean) {
					...
					// 对 getObject() 返回的对象进行代理，
					// 为什么需要对 getObject() 返回的对象进行代理？
					// 因为根据 spring 单例原则，需要 getObject() 返回的对象单例对象。
					return enhanceFactoryBean(factoryBean, beanMethod.getReturnType(), beanFactory, beanName);
				}
			}
			// 判断是 new 还是 get
			// 判断执行方法和调用方法是不是同一个方法，为 true 的话调用 proxy.invokeSuper(),否则的话通过 $$beanFactory.getBean() 出来
			if (isCurrentlyInvokedFactoryMethod(beanMethod)) {
				// The factory is calling the bean method in order to instantiate and register the bean
				// (i.e. via a getBean() call) -> invoke the super implem entation of the method to actually
			...
			// resolveBeanReference 该方法也是很重要的
		...
		private Object resolveBeanReference(Method beanMethod, Object[] beanMethodArgs,
			...
			// 判断对象是否正在创建，一个 bean 有三种状态，no（没有创建）、ing（创建中）、ed（创建成功）
			// 不管是 no 还是 ed spring都可以通过 getBean 拿到
