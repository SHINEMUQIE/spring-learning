...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	public void applyDefaults(BeanDefinitionDefaults defaults) {
		// 设置懒加载方式为默认懒加载模式，这里并没有把 bd 设置为懒加载，因为这个是后还没有 bd
		/**
		 * <beans lazy="true"> <bean> </bean>
		 * @Lazy(true)
		 * 当进入到该方法时，是一个循环 for (BeanDefinition candidate : candidates)，既第一次进入该方法扫描的是当前类，
		 * 但是并没有扫描全部的类，而这时 lazy="true" 只是作为一个属性被加载进来，当 拿到 bd 时，才将这个属性应用到这个 bd 上。
		 */
