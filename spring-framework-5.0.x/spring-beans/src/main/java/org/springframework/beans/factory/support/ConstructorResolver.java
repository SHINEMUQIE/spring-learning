...
class ConstructorResolver {
	...
	public BeanWrapper  autowireConstructor(String beanName, RootBeanDefinition mbd,
		 ...
		 * 因为BeanWrapper是个接口
		 * BeanWrapperImpl 中有一个 wrappedObject 属性，该属性值对应的是真是对象，
		 * 但是在这里真是的对象还没有被 new 出来，所以在这里为空
		 */
		 ...
		 * 代码就会执行到这里 spring 已经决定要采用一个特殊构造方法来实例 bean
		 * 但是到底用哪个？有的类可能提供了很多构造方法，但采用哪个，spring 有自己的一套规则。
		 * 当他找到一个之后他就会把这个构造方法赋值给 constructorToUse
		 */
		Constructor<?> constructorToUse = null;
		/**
		 * 构造方法的值，不是参数。构造方法通过反射来实例化一个对象，在调用反射来实例化对象的时候，需要具体的值
		 ...
		 * 确定参数列表
		 * argsToUse可以有两种办法设置
		 * 第一种通过 beanDefinition 设置
		 * 第二种通过xml设置
		 * 在这里 explicitArgs 一般为空，因为在这里还没有确定调用哪些构造方法。
		 * 而确定构造方法需要三个条件：
		 *  Constructor  构造方法
		 *  paramsType   参数类型
		 *  paramsValue	 参数值
		...
		if (explicitArgs != null) {
			...
			synchronized (mbd.constructorArgumentLock) {
				...
				// 档 spring 确定一个构造方法的时候，spring 会将构造方法赋值给 resolvedConstructorOrFactoryMethod，等需要用到时就可以直接取出来
		...
		if (constructorToUse == null) {
			...
			if (explicitArgs != null) {
				...
				// 实例一个对象，用来存放构造方法的参数
				// 当中主要存放了参数值和参数值所对应的表
				// ConstructorArgumentValues 用于存储构造方法的值的
				// 往构造参数中设值 mbd.getConstructorArgumentValues().addGenericArgumentValue("com.index.dao");
				// 拿到参数值
				/**
				 * 通过实现 BeanFactoryPostProcessor 的 postProcessBeanFactory 方法，往 UserService 的
				 * 构造参数中传递值，因为 Class 对象不会自动装配的，所以需要手动添加到容器中。同时在下面的代码中，
				 * createArgumentArray 方法会将该值转成一个对象。
				 * MyBatis 中的 MapperFactoryBean 也是通过这种方法加载的。
				 *
				 ...
				 *     </bean>
				 *
				 *     在通过 spring 内部给了一个值得情况那么表示你的构造方法的最小参数个数一定
				 *
				 * minNrOfArgs = 3
				 */
			...
			for (Constructor<?> candidate : candidates) {
				...
				// 判断当前参数列表中的参数个数是否小于最小参数个数，若小于，则终止本循环进入下次循环
				...
				if (resolvedValues != null) {
					try {
						/**
						 * 判断是否加入了ConstructorProperties注解如果加了则把值取出来
						...
						if (paramNames == null) {
							...
							if (pnd != null) {
								// 获取构造方法参数列表
								/**
								 * 假设你有一个（String muqie, Object zzz）
								 * 则paramNames=[muqie, zzz]
								 */
						 ...
						 * argsHolder 锁包含的值就是转换之后的
						 * spring 内部转换器,将 order 转化为具体的对象
						 * <constructor-org index="0" value="order" />
						 */
				 ...
				 * 为什么重要，因为需要清空
				 * 这也解释了为什么他找到两个符合要求的方法不一开始直接抛异常的原因
				 *
				 * 如果这个 ambiguousConstructors 一直存在，spring会在循环外面去exception
				 * 很牛逼呀！！！！
				 */
				...
				// 若找到相同的构造方法，将其放入到 ambiguousConstructors 中
				else if (constructorToUse != null && typeDiffWeight == minTypeDiffWeight) {
			...
			/**
			 * 如果 ambiguousConstructors 还存在则异常？为什么会在上面的方法中直接exception ？
			 * 以为 spring 在下面循环的过程中可能会找到更加合适的
			 * 看上面注释
			 */
			...
			if (explicitArgs == null) {
				/**
				 * 缓存相关信息：
				 * 1.已解析出构造方法对象 resolvedConstructorOrFactoryMethod
				 * 2.构造方法参数列表是否已解析标识：constructorArgumentsResolved
				 * 3.参数值列表 resolvedConstructorArgument 或 preparedConstructorArguments
				 * 这些信息可以在其他地方，用于进行快捷判断
				 */
		...
		try {
			//使用反射创建实例 lookup-method 通过CGLIB增强 bean 实例
