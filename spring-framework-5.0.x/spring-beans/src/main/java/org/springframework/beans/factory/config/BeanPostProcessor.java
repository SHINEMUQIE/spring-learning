 ...
 * (但是 spring 默认的怎么办？ 手动添加该类 set )
 * 		例如：在 org.springframework.context.annotation.AnnotatedBeanDefinitionReader#AnnotatedBeanDefinitionReader 方法中实现了
 * 	AnnotationConfigUtils.registerAnnotationConfigProcessors(this.registry); 该方法就是把 spring 自己的一些后置处理器注册到 bean 中，六个。
 * 	既，org.springframework.beans.factory.support.DefaultListableBeanFactory#beanDefinitionNames spring 启动时加载到 beanDefinitionNames 中的 bd。
